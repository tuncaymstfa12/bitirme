...
import { showModal, closeModal, showToast, formatDate, daysUntil, renderStars, formatLocalDate } from './components.js';
import { getTopicsForLesson, getLessonsForTrack, EXAM_STRUCTURE, calcNet } from '../data/curriculum.js';

export function renderExamManager(container) {
  var exams = store.getExams();
  var topics = store.getTopics();

  var orphanTopics = topics.filter(function(tp){ return !exams.some(function(e){ return e.id === tp.examId; }); });
  var html = [
    ...
    '<button class="btn btn-secondary btn-lg" id="add-topic-top-btn">'+t('exams.addTopicTop')+'</button>',
    '<button class="btn btn-secondary btn-lg" id="add-mock-btn">📝 '+t('exams.addMock')+'</button>',
    '</div>'
  ];

  if (exams.length === 0) {
    html.push('<div class="card"><div class="empty-state"><div class="empty-icon">🎓</div><h3>'+t('exams.noExams')+'</h3><p>'+t('exams.noExamsDesc')+'</p><button class="btn btn-primary" id="add-exam-btn-empty">'+t('exams.addFirstExam')+'</button></div></div>');
  } else {
    exams.forEach(function(ex) {
      var d = daysUntil(ex.date);
      var examTopics = topics.filter(function(tp){ return tp.examId === ex.id; });
      html.push(
        '<div class="card" style="margin-bottom:var(--space-md);border-left:4px solid '+ex.color+';">',
        '<div style="display:flex;align-items:center;gap:var(--space-md);padding:var(--space-md);">',
        '<div style="flex:1;min-width:0;"><div style="font-weight:700;font-size:var(--font-lg);">'+ex.name+'</div><div style="font-size:var(--font-xs);color:var(--text-tertiary);">'+formatDate(ex.date)+' · '+(d>=0?t('exams.daysLeft',{n:d}):t('exams.pastExam'))+'</div></div>',
        '<button class="btn btn-sm btn-secondary add-topic-exam-btn" data-exam-id="'+ex.id+'" title="'+t('exams.addTopicTop')+'">+</button>',
        '<button class="btn btn-sm btn-secondary edit-exam-btn" data-exam-id="'+ex.id+'">✎</button>',
        '<button class="btn btn-sm btn-danger delete-exam-btn" data-exam-id="'+ex.id+'">🗑</button>',
        '</div>'
      );
      if (examTopics.length === 0) {
        html.push('<div style="padding:0 var(--space-md) var(--space-md);color:var(--text-tertiary);font-size:var(--font-sm);">'+t('exams.noTopicsAdded')+'</div>');
      } else {
        html.push('<div style="padding:0 var(--space-md) var(--space-md);display:flex;flex-direction:column;gap:var(--space-xs);">');
        examTopics.forEach(function(tp) {
          var mr = store.getMockResults(tp.id);
          html.push(
            '<div style="display:flex;align-items:center;gap:var(--space-sm);padding:var(--space-xs) var(--space-sm);border-radius:var(--radius-sm);background:var(--bg-secondary);">',
            '<div style="flex:1;min-width:0;">',
            '<div style="font-weight:600;font-size:var(--font-sm);">'+tp.name+(tp.examType?' <span style="color:var(--text-tertiary);font-size:var(--font-xs);">'+tp.examType+(tp.lesson?' — '+tp.lesson:'')+'</span>':'')+'</div>',
            '<div style="font-size:var(--font-xs);color:var(--text-tertiary);display:flex;gap:var(--space-sm);flex-wrap:wrap;">',
            '<span>'+t('exams.weight')+': '+tp.weight+'/10</span>',
            '<span>'+t('exams.self')+': '+renderStars(tp.selfAssessment)+'</span>',
            '<span>'+t('exams.est')+': '+tp.estimatedMinutes+t('exams.min')+'</span>',
            (mr.length?'<span>'+t('exams.tests')+': '+mr.length+'</span>':''),
            '</div></div>',
            '<button class="btn btn-sm btn-secondary edit-topic-top-btn" data-topic-id="'+tp.id+'" style="padding:2px 6px;font-size:var(--font-xs);">✎</button>',
            '<button class="btn btn-sm btn-danger delete-topic-top-btn" data-topic-id="'+tp.id+'" style="padding:2px 6px;font-size:var(--font-xs);">🗑</button>',
            '</div>'
          );
        });
        html.push('</div>');
  ...
  if (orphanTopics.length > 0) {
    html.push('<div class="card" style="margin-bottom:var(--space-md);border-left:4px solid var(--text-tertiary);">');
    html.push('<div style="padding:var(--space-md);font-weight:700;font-size:var(--font-lg);color:var(--text-tertiary);">'+t('exams.topicsSection')+' ('+t('exams.noExams')+')</div>');
    html.push('<div style="padding:0 var(--space-md) var(--space-md);display:flex;flex-direction:column;gap:var(--space-xs);">');
    orphanTopics.forEach(function(tp) {
      var mr = store.getMockResults(tp.id);
      html.push(
        '<div style="display:flex;align-items:center;gap:var(--space-sm);padding:var(--space-xs) var(--space-sm);border-radius:var(--radius-sm);background:var(--bg-secondary);">',
        '<div style="flex:1;min-width:0;">',
...
function bindEvents(container) {
  ...
  var at = container.querySelector('#add-topic-top-btn'); if(at)at.addEventListener('click',function(){showTopicForm(null);});
  ...
  container.querySelectorAll('.add-topic-exam-btn').forEach(function(btn){
    btn.addEventListener('click',function(){showTopicForm(btn.dataset.examId);});
  });
  container.querySelectorAll('.edit-topic-top-btn').forEach(function(btn){
    btn.addEventListener('click',function(){var tp=store.getTopic(btn.dataset.topicId);if(tp)showTopicForm(tp.examId,tp);});
  });
  container.querySelectorAll('.delete-topic-top-btn').forEach(function(btn){
    btn.addEventListener('click',function(){
...
function showExamForm(existing){
  var isEdit=!!existing;
  var dd=new Date();dd.setDate(dd.getDate()+14);var ds=formatLocalDate(dd);
  var ov=showModal({
...
function showTopicForm(examId,existing){
  ...
  ov.querySelector('#modal-save').addEventListener('click',function(){
    ...
    var selectedExam=ov.querySelector('#t-exam').value;
...
function showMockResultForm(){
  ...
  function buildTable(){
    ...
    var rows='', langSel='';
    if(exType==='dil'){
      langSel = '<div class="form-group"><label class="form-label">'+t('exams.language')+'</label><select class="form-select" id="mock-lang">' +
    ...
    str.lessons.forEach(function(l,i){
      ...
      rows += '<tr><td style="font-weight:600;">'+name+'</td>'+
        '<td style="text-align:center;color:var(--text-secondary);">'+l.questions+' '+t('exams.questionsCol').toLowerCase()+'</td>'+
    ...
    return langSel +
      '<div style="overflow-x:auto;"><table class="data-table" style="width:100%;"><thead><tr><th>'+t('exams.lessonCol')+'</th><th>'+t('exams.questionsCol')+'</th><th>'+t('exams.correctCol')+'</th><th>'+t('exams.wrongCol')+'</th><th>'+t('exams.netCol')+'</th><th>%</th></tr></thead><tbody id="mock-rows">'+rows+'</tbody></table></div>'+
      '<div style="margin-top:var(--space-md);text-align:right;font-weight:700;">'+t('exams.totalNet')+': <span id="mock-total-net">0.00</span></div>'+
      '<small style="color:var(--text-tertiary);">'+t('exams.wrongDeductNote')+'</small>';
  }

  var ov=showModal({
    title:'📝 '+t('exams.mockTitle'),
    content:
      '<div class="form-group"><label class="form-label">'+t('exams.mockTypeLabel')+'</label><select class="form-select" id="mock-type">'+
      '<option value="TYT"'+(exType==='TYT'?' selected':'')+'>TYT (120 soru)</option>'+
      '<option value="sayisal"'+(exType==='sayisal'?' selected':'')+'>AYT '+t('exams.trackSayisal')+' (80 soru)</option>'+
      '<option value="esit_agirlik"'+(exType==='esit_agirlik'?' selected':'')+'>AYT '+t('exams.trackEa')+' (80 soru)</option>'+
      '<option value="sozel"'+(exType==='sozel'?' selected':'')+'>AYT '+t('exams.trackSozel')+' (80 soru)</option>'+
      '<option value="dil"'+(exType==='dil'?' selected':'')+'>YDT '+t('exams.trackDil')+' (80 soru)</option>'+
      '</select></div>'+
      '<div class="form-group"><label class="form-label">'+t('exams.mockDateLabel')+'</label><input type="date" class="form-input" id="mock-date" value="'+formatLocalDate(new Date())+'"></div>'+
      '<div id="mock-table">'+buildTable()+'</div>',
    footer:'<button class="btn btn-secondary" id="modal-cancel">'+t('exams.cancel')+'</button><button class="btn btn-primary" id="modal-save">'+t('exams.saveResult')+'</button>'
  });
  ...
  ov.querySelector('#modal-save').addEventListener('click',function(){
    ...
    rows.forEach(function(r){
      var topicName = r.lesson;
      var matchingTopic = topics.find(function(tp){ return tp.name === topicName || tp.lesson === topicName; });
      if(matchingTopic){
        store.addMockResult(createMockResult({topicId:matchingTopic.id,score:Math.round(r.net*100)/100,maxScore:r.total*100,date:date}));
      }
    });
    ...
    showToast({title:t('exams.mockSaved'),message:t('exams.lessonsCount',{n:saved})+' · '+t('exams.netCol')+': '+totalNet.toFixed(2)+'/'+totalQ+' (%'+overallPct+') — '+getNetLabel(overallPct),type:'success'});
    closeModal(); var c=document.getElementById('main-content'); if(c)renderExamManager(c);
  });
}

function getNetLabel(pct){if(pct>=90)return t('exams.veryGood');if(pct>=80)return t('exams.good');if(pct>=60)return t('exams.average');if(pct>=40)return t('exams.weak');return t('exams.veryWeak');}